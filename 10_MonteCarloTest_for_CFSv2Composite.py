...
def CompositesSimple_CFSv2(data, index):
    try:
        data_sel = data.sel(position=index, drop=True)
    except:
        print('Error select')
        print('Trying wo drop')
        data_sel = data.sel(position=index)
    return data_sel.mean('position')
...
for v in variables:
    ...
    neutro['time'] = np.arange(len(neutro.time))
        ...
        def PermuDatesComposite(n, data=concat, len_neutro=len_neutro):
            ...
            for a in n:
                ...
                if a == n[0]:
                    ...
                    comp_concat = xr.concat([comp_concat, comp], dim='position')
            ...
            del neutro_comp, event_comp, comp
